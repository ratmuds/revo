...
// Read AS5600 OUT pin (voltage mode)
  ...
  if (Serial.available()) {
    ...
    if (!error && doc["magic"] == "REVO") {
      ...
      if (servos.size() == 7) {
        ...
        // ----------------------------------------------------------------------
        // Update Local Hardware PWM Lines (IDs 4, 5, 6)
        // ----------------------------------------------------------------------
        ...
        joints[6].angle = servos[6]["a"].as<uint16_t>(); // this one no have sensor :(((
        ...
        // ----------------------------------------------------------------------
        // Query Single RS-485 Target Component via the Serial Pipeline
        // ----------------------------------------------------------------------
        JsonObject currentRs485Cmd = servos[currentPcbId];
        
        // Wrap JSON into structured binary data
        ...
        // Release line back to Receive Mode 
        digitalWrite(RS485_DE, LOW);   
